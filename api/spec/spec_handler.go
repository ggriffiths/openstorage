...
var (
	...
	tokenSecretPublicDataRegex  = regexp.MustCompile(api.TokenSecretPublicData + `=/*([0-9A-Za-z_/]+),?`)
	tokenSecretCustomDataRegex  = regexp.MustCompile(api.TokenSecretCustomData + `=/*([0-9A-Za-z_/]+),?`)
...
func (d *specHandler) GetTokenSecretContextFromString(str string) (*api.TokenSecretContext, bool) {
	...
	_, secretPublicData := d.getVal(tokenSecretPublicDataRegex, str)
	_, secretCustomData := d.getVal(tokenSecretCustomDataRegex, str)
