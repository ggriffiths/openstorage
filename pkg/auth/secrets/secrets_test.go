...
func TestGetToken(t *testing.T) {
	tt := []struct {
		...
		{
			testName: "azure get token",

			secretType: TypeAzure,
			secretName: "secret-name-azure",
			token:      "auth-token",
		...
		{
			testName: "gcloud get token",

			secretType: TypeGCloud,
			secretName: "secret-name-gcloud",
			token:      "auth-token",

			expectError: false,
		},
		{
			testName: "ibm get token",

			secretType: TypeIBM,
			secretName: "secret-name-ibm",
			token:      "auth-token",
		...
		{
			testName: "aws get token",

			secretType: TypeAWS,
			secretName: "secret-name-aws",
			token:      "auth-token",
			publicData: false,
