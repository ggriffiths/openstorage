...
const (
	...
	// SecretPublicDataKey is a label used on the openstorage.Volume object
	// which corresponds to whether or not a secret is marked as public data.
	SecretPublicDataKey = "openstorage.io/auth-secret-public-data"

	// SecretCustomDataKey is a label used on the openstorage.Volume object
	// which corresponds to whether or not a secret is marked as custom data.
	SecretCustomDataKey = "openstorage.io/auth-secret-custom-data"
...
func (a *Auth) requestToContext(request *api.TokenSecretContext) map[string]string {
	...
	if request.SecretCustomData != "" {
		context[osecrets.CustomSecretData] = request.SecretCustomData
	}
	if request.SecretPublicData != "" {
		context[osecrets.PublicSecretData] = request.SecretPublicData
	}
